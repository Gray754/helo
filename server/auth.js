...
const login = (req, res) =>{
    ...
    db.findUser(req.body.username).then(async result=>{
        ...
        if(!result.length){
            ...
            if(isMatch){
                req.session.user={username: result[0].username, id: result[0].user_id}
...
const register = async (req, res)=>{
    ...
    try{
        ...
        req.session.user={username: response[0].username, id: response[0].user_id}
        res.json(req.session.user)
    }catch(err){
        console.log(err)
